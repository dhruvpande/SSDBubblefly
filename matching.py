...
def get_top_confidence(pred_labels,top_k):
	confidence =[]

	for logits in pred_labels:
		probs=np.exp(logits)/(np.sum(np.exp(logits))+1e-3)
		top_label=np.amax(probs)
		confidence.append(top_label)

	k=min(top_k,confidence)
	top_confidences=np.argpartition(np.asarray(confidence),k)[-k:]
...
class Matcher:
    ...
    def match_boxes(self,pred_labels,anns):
    	matches=[[[[None for i in range(c.layer_boxes[o])] for x in range(c.out_shapes[o][1])]for y in range(c.out_shapes[o][2])]for o in range(len(layer_boxes))]
    	...
    	for index, (gt_box,id) in zip(range(len(anns)),anns):
    		...
    		for o in range(len(layer_boxes)):
    			...
    			for y in range(y1,y2):
    				for x in range(x1,x2):
    					for i in range(len(layer_boxes)):
            ...
            confidences=get_top_confidence(pred_labels,negative_max)
